...
class Search extends React.Component {
  ...
  search() {
    ...
    if (this.state.query) {
      ...
      var fuse = new Fuse(corpus, options) // "list" is the item array
      searchResults = fuse.search(this.state.query)
    }
    ...
    if (searchResults && searchResults.length > 0) {
      this.setState({
        ...
        indexStart: 0,
        indexEnd: Math.min(9, searchResults.length - 1)
      })
    } else {
      ...
      <Container {...this.props}>
        ...
        <section className="sub-header__wrapper">
          <section className="sub-header">
            <span className="sub-header__title sub-header-title__fixed">
              Search results
            </span>
          </section>
        </section>
        <SearchPageLayout>
          ...
          <BottomSticky zIndex={10} horizontalAlign="right">
            <TabletHidden>
              <Feedback
                onSubmit={data =>
                  submitFeedback(this.props.location.pathname, data)
                }
              />
            </TabletHidden>
          </BottomSticky>
