...
import { storeComment, storeEmail, storeFeedback, storeMissing } from './firebase';
...
exports.submitMissing = functions.https.onRequest((req, res) => {
    corsInstance(req, res, async () => {
        const packet: IMissing = req.body;
        if (
            packet === undefined ||
            packet.document === undefined
        ) {
            console.log('submitMissing failed. Packet: ', packet);
            return res.status(400).json({ message: 'Ensure all fields are included' });
        }

        try {
            return res.json({ message: await storeMissing(packet) });
        } catch (error) {
            console.log('submitMissing failed. Error: ', error);
            return res.status(403).json({ error: error.toString() });
        }
    });
});
