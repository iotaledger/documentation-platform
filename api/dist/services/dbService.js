"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
var dbHelper_1 = require("../utils/dbHelper");
/**
 * Service to handle db requests.
 */
var DbService = /** @class */ (function () {
    function DbService(config, tableName, idName) {
        this._config = config;
        this._tableName = tableName;
        this._idName = idName;
    }
    /**
     * Create the table for the items.
     */
    DbService.prototype.createTable = function () {
        return __awaiter(this, void 0, void 0, function () {
            var log, dbConnection, tableParams;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        log = "Creating table " + this._tableName + "\n";
                        dbConnection = dbHelper_1.DbHelper.createConnection(this._config);
                        tableParams = {
                            AttributeDefinitions: [
                                {
                                    AttributeName: this._idName,
                                    AttributeType: "S"
                                }
                            ],
                            KeySchema: [
                                {
                                    AttributeName: this._idName,
                                    KeyType: "HASH"
                                }
                            ],
                            ProvisionedThroughput: {
                                ReadCapacityUnits: 1,
                                WriteCapacityUnits: 1
                            },
                            TableName: this._tableName
                        };
                        return [4 /*yield*/, dbConnection.createTable(tableParams).promise()];
                    case 1:
                        _a.sent();
                        log += "Waiting for " + this._tableName + "\n";
                        return [4 /*yield*/, dbConnection.waitFor("tableExists", {
                                TableName: this._tableName
                            }).promise()];
                    case 2:
                        _a.sent();
                        log += this._tableName + " Created Successfully\n";
                        return [2 /*return*/, log];
                }
            });
        });
    };
    /**
     * Get the item.
     * @param id The id of the item to get.
     */
    DbService.prototype.get = function (id) {
        return __awaiter(this, void 0, void 0, function () {
            var docClient, key, response, err_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        _a.trys.push([0, 2, , 3]);
                        docClient = dbHelper_1.DbHelper.createDocClient(this._config);
                        key = {};
                        key[this._idName] = id;
                        return [4 /*yield*/, docClient.get({
                                TableName: this._tableName,
                                Key: key
                            }).promise()];
                    case 1:
                        response = _a.sent();
                        return [2 /*return*/, response.Item];
                    case 2:
                        err_1 = _a.sent();
                        return [3 /*break*/, 3];
                    case 3: return [2 /*return*/];
                }
            });
        });
    };
    /**
     * Set the item.
     * @param item The item to set.
     */
    DbService.prototype.set = function (item) {
        return __awaiter(this, void 0, void 0, function () {
            var docClient;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        docClient = dbHelper_1.DbHelper.createDocClient(this._config);
                        return [4 /*yield*/, docClient.put({
                                TableName: this._tableName,
                                Item: item
                            }).promise()];
                    case 1:
                        _a.sent();
                        return [2 /*return*/];
                }
            });
        });
    };
    return DbService;
}());
exports.DbService = DbService;
